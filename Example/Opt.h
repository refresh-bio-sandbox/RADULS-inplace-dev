...
template<typename T>
struct SaveOpt<std::optional<T>>
{
	static void Save(const std::string& in, Opt<std::optional<T>>& opt)
	{
		std::istringstream iss(in.substr(opt.str.length()));
		T v;
		iss >> v;
...
class Params
{
	...
	template<typename T>
	void PrintOpt(const Opt<std::optional<T>>& opt)
	{
		if (opt.val)
			std::cerr << "  " << std::setw(18) << std::left << opt.str << " - " << *opt.val << "\n";
		else
			std::cerr << "  " << std::setw(18) << std::left << opt.str << " - " << "no set" << "\n";
...
public:
	...
	Opt<std::optional<uint32_t>> n_phases = Opt<std::optional<uint32_t>>("-n_phases", "number of most significant bytes of a key that will be used to sort (default: all bytes)");
	...
	Opt<bool> all_thread_to_gen_data = Opt<bool>("-all_gen", "use only n_threads to generate radnom data", true);
	Params(int argc, char **argv)
	{
		...
		for (int i = 0; i < argc; ++i)
		{
			...
			set_option(in, n_phases);
			set_option(in, full_validation);			
			set_option(in, clear_tmp);
			set_option(in, all_thread_to_gen_data);
			set_option(in, input);
		...
		if (n_phases.val && *n_phases.val > key_size.val)
		{
			std::cerr << "Error: n_phases must be lower or equal to key_size\n";
			err = true;
		}
	...
	void Help(const std::string& exe)
	{
		...
		HelpForOpt(all_thread_to_gen_data);
	...
	void Print()
	{
		...
		PrintOpt(clear_tmp);
		PrintOpt(all_thread_to_gen_data);
		PrintOpt(input);
		PrintOpt(output);
	}
};
